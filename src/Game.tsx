...
export default function Game(props: { dialogOpened: boolean, openDialog: (variant:DialogType)=> void }): JSX.Element {
  ...
  return (
    <div id='main'>
      <Board updatePoints={()=>setPoints(pt=>pt+1)} board={board} setBoard={setBoard} dialogOpened={props.dialogOpened}/>
      <SidePane score={points}>
        ...
        <button type='button' onClick={()=> setBoard(generateBoard())}> New Game </button>
      </SidePane>
    </div>
  )
}